...
// VibeWriteSubscribePrice handles GET /api/vibe-write/subscribe-price
// Returns the Pro subscription price in both USDT and BNB.
func VibeWriteSubscribePrice(c *gin.Context) {
	...
	if chain.C != nil {
		...
		if err == nil && bnbPrice > 0 {
			...
			util.Log.Warn("[vibe-write] Failed to get BNB price: %v", err)
			resp["bnb_price"] = 0
			resp["price_bnb"] = "0"
		}
	}
...
// VibeWriteSubscribe handles POST /api/vibe-write/subscribe
// Creates or renews a Vibe Write subscription.
// Supports payment in USDT (ERC-20 transfer) or BNB (native transfer) to TREASURY_ADDR.
func VibeWriteSubscribe(c *gin.Context) {
...
// VibeWriteGetSubscription handles GET /api/vibe-write/subscription
// Returns the user's subscription status.
func VibeWriteGetSubscription(c *gin.Context) {
	walletAddr := middleware.GetSessionWallet(c)
	if walletAddr == "" {
		c.JSON(http.StatusUnauthorized, gin.H{"error": "login required"})
		return
...
// VibeWriteAddKOL handles POST /api/vibe-write/kols
// Adds a KOL to the user's tracking list.
func VibeWriteAddKOL(c *gin.Context) {
	walletAddr := middleware.GetSessionWallet(c)
	if walletAddr == "" {
		c.JSON(http.StatusUnauthorized, gin.H{"error": "login required"})
		return
	...
	kol, err := services.AddVibeWriteKOL(walletAddr, cleanHandle)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
...
// VibeWriteRemoveKOL handles DELETE /api/vibe-write/kols/:id
// Removes a KOL from the user's tracking list.
func VibeWriteRemoveKOL(c *gin.Context) {
	walletAddr := middleware.GetSessionWallet(c)
	if walletAddr == "" {
		c.JSON(http.StatusUnauthorized, gin.H{"error": "login required"})
		return
	...
	if err := services.RemoveVibeWriteKOL(walletAddr, kolID); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
...
// VibeWriteListKOLs handles GET /api/vibe-write/kols
// Returns the user's tracked KOLs.
func VibeWriteListKOLs(c *gin.Context) {
	walletAddr := middleware.GetSessionWallet(c)
	if walletAddr == "" {
		c.JSON(http.StatusUnauthorized, gin.H{"error": "login required"})
		return
	...
	kols, err := services.ListVibeWriteKOLs(walletAddr)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
...
// VibeWriteGenerateReply handles POST /api/vibe-write/reply
// Generates reply suggestions for a specific tweet.
func VibeWriteGenerateReply(c *gin.Context) {
	walletAddr := middleware.GetSessionWallet(c)
	if walletAddr == "" {
		c.JSON(http.StatusUnauthorized, gin.H{"error": "login required"})
		return
...
// VibeWriteGetReplies handles GET /api/vibe-write/replies
// Returns the user's recent generated replies.
func VibeWriteGetReplies(c *gin.Context) {
	walletAddr := middleware.GetSessionWallet(c)
	if walletAddr == "" {
		c.JSON(http.StatusUnauthorized, gin.H{"error": "login required"})
		return
...
// VibeWriteSetPersona handles POST /api/vibe-write/persona
// Creates or updates the user's persona for reply generation.
func VibeWriteSetPersona(c *gin.Context) {
	walletAddr := middleware.GetSessionWallet(c)
	if walletAddr == "" {
		c.JSON(http.StatusUnauthorized, gin.H{"error": "login required"})
		return
...
// VibeWriteGetPersona handles GET /api/vibe-write/persona
// Returns the user's persona.
func VibeWriteGetPersona(c *gin.Context) {
	walletAddr := middleware.GetSessionWallet(c)
	if walletAddr == "" {
		c.JSON(http.StatusUnauthorized, gin.H{"error": "login required"})
		return
