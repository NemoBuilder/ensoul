...
// ──────────────────────────────────────────────────────────────────────────────
// Tag Service — Vibe Write 2.0 tag-based content system
// ──────────────────────────────────────────────────────────────────────────────

// TagWithAccounts is the response type for GET /api/vibe-write/tags.
...
func UpdateUserSelectedTags(walletAddr string, tagIDs []string) error {
	...
	var count int64
	database.DB.Model(&models.VibeWriteTag{}).Where("id IN ? AND active = ?", tagIDs, true).Count(&count)
	...
	tx.Commit()
	util.Log.Info("[vibe-write] Updated tag selections for %s: %v", walletAddr, tagIDs)
	return nil
}
...
func MuteAccount(walletAddr, handle string) error {
	...
	util.Log.Info("[vibe-write] %s muted @%s", walletAddr, handle)
	return nil
}
...
func UnmuteAccount(walletAddr, handle string) error {
	...
	util.Log.Info("[vibe-write] %s unmuted @%s", walletAddr, handle)
	return nil
}
...
func AdminCreateTag(tag *models.VibeWriteTag) error {
	if tag.ID == "" {
...
func AdminUpdateTag(tagID string, updates map[string]interface{}) error {
	result := database.DB.Model(&models.VibeWriteTag{}).Where("id = ?", tagID).Updates(updates)
	if result.RowsAffected == 0 {
...
func AdminAddAccountToTag(tagID, handle, displayName string, realtimePriority bool) error {
	...
	util.Log.Info("[vibe-write-admin] Added @%s to tag %s", handle, tagID)
	return nil
}
...
func AdminRemoveAccountFromTag(tagID, handle string) error {
	result := database.DB.Where("tag_id = ? AND handle = ?", tagID, handle).
		Delete(&models.VibeWriteTagAccount{})
	if result.RowsAffected == 0 {
...
func AdminDeleteTag(tagID string) error {
	...
	database.DB.Where("tag_id = ?", tagID).Delete(&models.VibeWriteTagAccount{})

	// Set tag inactive
	tag.Active = false
	database.DB.Save(&tag)

	util.Log.Info("[vibe-write-admin] Deleted tag %s (%s)", tagID, tag.Name)
	return nil
}

// AdminListTagAccounts returns all accounts for a given tag.
func AdminListTagAccounts(tagID string) ([]models.VibeWriteTagAccount, error) {
	var accounts []models.VibeWriteTagAccount
	...
	for _, handle := range handles {
		...
		if SocialDataAvailable() {
			...
			if err == nil {
				...
				util.Log.Warn("[vibe-write-admin] Failed to fetch profile for @%s: %v", handle, err)
			}
		}
		...
		if err := database.DB.Create(&candidate).Error; err != nil {
			util.Log.Warn("[vibe-write-admin] Failed to create candidate for @%s: %v", handle, err)
			continue
		}
		imported++
	}

	util.Log.Info("[vibe-write-admin] Imported %d candidates (source: %s)", imported, source)
	return imported, nil
}

// AdminListCandidates returns candidates with optional status filter.
func AdminListVibeWriteCandidates(status string, limit, offset int) ([]models.TagCandidate, int64, error) {
...
func AdminApproveCandidate(candidateID uuid.UUID, approvedTagIDs []string, realtimePriority bool, adminWallet string) error {
	...
	for _, tagID := range approvedTagIDs {
		...
		if err != nil {
			util.Log.Warn("[vibe-write-admin] Approve: failed to add @%s to tag %s: %v", candidate.Handle, tagID, err)
		}
	}
	...
	util.Log.Info("[vibe-write-admin] Approved candidate @%s → tags %v", candidate.Handle, approvedTagIDs)
	return nil
}
...
func AdminRejectCandidate(candidateID uuid.UUID, adminWallet string) error {
	...
	util.Log.Info("[vibe-write-admin] Rejected candidate @%s", candidate.Handle)
	return nil
}
...
- If the account doesn't clearly fit any tag, return empty tags array`, handle, displayName, bio, tagList)
	...
	err := CallVibeWriteLLMJSON([]ChatMessage{
		{Role: "system", Content: "You are a precise classifier. Output valid JSON only."},
		{Role: "user", Content: prompt},
	}, 500, 0.3, &result)

	if err != nil {
		util.Log.Warn("[vibe-write-admin] LLM classification failed for @%s: %v", handle, err)
