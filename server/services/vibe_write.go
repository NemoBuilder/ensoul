...
func GenerateReplies(walletAddr, handle, tweetID, tweetText string) (*models.VibeWriteReply, error) {
	return Snipe(walletAddr, handle, tweetID, tweetText, "", "")
}

// Snipe generates reply suggestions for any tweet (Vibe Write 2.0).
...
func Snipe(walletAddr, authorHandle, tweetID, tweetText, tagID, language string) (*models.VibeWriteReply, error) {
	...
	util.Log.Info("[vibe-write] Snipe: generated %d replies for @%s tweet %s (user: %s, soul: %v)",
...
`, persona.Bio, persona.Style, persona.Materials, language)
	...
	prompt := fmt.Sprintf(`You are a reply generation engine for Vibe Write.
...
// GetUserReplies returns the user's recent vibe-write replies.
func GetUserReplies(walletAddr string, limit int) ([]models.VibeWriteReply, error) {
	if limit <= 0 || limit > 100 {
		limit = 20
	}

	var replies []models.VibeWriteReply
...
// MonitorKOLTweets is DEPRECATED — replaced by tag-based feed refresher in Vibe Write 2.0.
...
func MonitorKOLTweets() {
	...
	util.Log.Info("[vibe-write] Monitoring %d unique KOLs across %d subscriptions", len(handleToSubs), len(subs))

	for handle, subscribers := range handleToSubs {
		tweets, err := fetchRecentTweets(handle)
		if err != nil {
			util.Log.Warn("[vibe-write] Failed to fetch tweets for @%s: %v", handle, err)
			continue
		}
			...
			for _, sub := range subscribers {
				...
				go func(wallet, h, tID, tText string) {
					if _, err := GenerateReplies(wallet, h, tID, tText); err != nil {
						util.Log.Warn("[vibe-write] Auto-reply failed for @%s tweet %s: %v", h, tID, err)
					}
...
// StartVibeWriteMonitor is DEPRECATED — replaced by StartFeedRefresher() in Vibe Write 2.0.
func StartVibeWriteMonitor(interval time.Duration) {
	go func() {
	...
	util.Log.Info("[vibe-write] KOL tweet monitor started (interval: %s)", interval)
}
