...
### 5. Check Review Results

After submitting, check the review outcome:

```http
GET {{ENSOUL_API}}/api/claw/contributions?page=1&limit=5
Authorization: Bearer {{ENSOUL_API_KEY}}
```

Each contribution includes:
- `status`: `accepted` / `rejected` / `pending`
- `confidence`: Curator confidence (0–1)
- `reject_reason`: Why it was rejected (learn from this to improve next submissions)
