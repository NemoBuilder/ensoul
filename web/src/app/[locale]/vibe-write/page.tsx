...
import TagCloudFilter from "@/components/vibe-write/TagCloudFilter";
import TweetFeed from "@/components/vibe-write/TweetFeed";
import SubscribeModal from "@/components/vibe-write/SubscribeModal";
import { useVibeWriteSSE, type SSEStatus } from "@/components/vibe-write/useVibeWriteSSE";
...
export default function VibeWritePage() {
  ...
  // Tags
  const [tags, setTags] = useState<VibeWriteTag[]>([]);
  ...
  const [snipeResults, setSnipeResults] = useState<Record<string, VibeWriteReply | null>>({});
  ...
  useEffect(() => {
    ...
    async function init() {
      try {
        ...
        const tagData = await vibeWriteApi.getTags();
        if (cancelled) return;
        ...
        if (isConnected && address) {
          ...
          if (hasSession) {
            ...
            const [userTagsRes, subRes, mutedRes, repliesRes, personaRes] = await Promise.allSettled([
              vibeWriteApi.getUserTags(),
              vibeWriteApi.getSubscription(),
              vibeWriteApi.getMuted(),
              vibeWriteApi.getReplies(),
              vibeWriteApi.getPersona(),
            ]);
            if (cancelled) return;
  ...
  const loadFeed = useCallback(
    async (tagIds: string[], append = false, pageCursor?: string) => {
      ...
      try {
        const result = await vibeWriteApi.getFeed(tagIds, pageCursor);
  ...
  const { status: sseStatus } = useVibeWriteSSE({
  ...
  function handleToggleTag(tagId: string) {
    setSelectedTagIds((prev) => {
      ...
      if (isConnected) {
        vibeWriteApi.updateUserTags(next).catch(() => {});
      }
      return next;
    });
  }
  ...
  async function handleSnipe(tweet: TweetCardType) {
    ...
    try {
      const result = await vibeWriteApi.snipe(
        tweet.id,
        tweet.text,
        tweet.author.handle,
        tweet.tags[0] || "",
      ...
      try {
        const sub = await vibeWriteApi.getSubscription();
        setSubscription(sub);
      } catch {}
    } catch (err: unknown) {
      setSnipeErrors((prev) => ({
  ...
  async function handleMute(handle: string) {
    ...
    if (isConnected) {
      try {
        await vibeWriteApi.muteAccount(handle);
      } catch {
  ...
  function handleSubscribeSuccess() {
    ...
    vibeWriteApi.getSubscription().then(setSubscription).catch(() => {});
