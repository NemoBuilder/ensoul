...
export default function VibeWriteSettingsPage() {
  ...
  async function handleToggleTag(tagId: string) {
    ...
    try {
      await vibeWriteApi.updateUserTags(next);
    } catch {}
  }
  ...
  async function handleUnmute(handle: string) {
    ...
    try {
      await vibeWriteApi.unmuteAccount(handle);
    } catch {
      // Revert
  ...
  async function handleSavePersona() {
    ...
    setPersonaMsg("");
    try {
      await vibeWriteApi.setPersona(pBio, pStyle, pMaterials, pLanguage);
    ...
    <div className="mx-auto max-w-2xl px-4 pt-20 pb-16">
      ...
      <SubscribeModal
        ...
        onSuccess={() => {
          ...
          vibeWriteApi.getSubscription().then((s) => setSubscription(s)).catch(() => {});
        }}
      />
    </div>
  );
