...
export default function VibeWriteTagsPage() {
  ...
  const loadTags = useCallback(async () => {
    try {
      setLoading(true);
      const res = await adminvibeWriteApi.listTags();
      setTags(res.tags || []);
    } catch (err: unknown) {
  ...
  const handleEditTag = (tag: VibeWriteTag) => {
    setEditingTag({
      id: tag.id,
      name: tag.name,
      name_en: tag.name_en,
  ...
  const handleSaveTag = async (data: TagFormData) => {
    ...
    try {
      if (editingTag) {
        // Update existing tag
        await adminvibeWriteApi.updateTag(data.id, {
          name: data.name,
        ...
        // Create new tag
        await adminvibeWriteApi.createTag(data);
  ...
  const handleDeleteTag = async (tagId: string, tagName: string) => {
    if (!confirm(`Delete tag "${tagName}" (${tagId})? This will remove all associated accounts.`))
      return;
    try {
      await adminvibeWriteApi.deleteTag(tagId);
  ...
  const handleAddAccount = async (handle: string, displayName: string, realtime: boolean) => {
    ...
    setError("");
    try {
      await adminvibeWriteApi.addTagAccount(addAccountTagId, handle, displayName, realtime);
  ...
  const handleRemoveAccount = async (tagId: string, handle: string) => {
    ...
    try {
      await adminvibeWriteApi.removeTagAccount(tagId, handle);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold text-[#e2e8f0]">🏷️ Vibe Write Tags &amp; KOL Accounts</h2>
